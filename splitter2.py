f = open(r'C:\Users\User\Desktop\data\data2.txt', encoding="utf8")
...
def split():
    ...
    for line in data:
        ...
        if ((words[0][0].isupper()and words[0][1].isdigit()) or words[0][0].isdigit()) and len(words[0])>=3 : # check for not 2 lines description
            words.insert(1,"-")
            words.insert(2,"AX 23PBX Endoscopy") 
            doc.append(words)
            x = x+1
        else :
...
def write():
    global x
    w=open(r'C:\Users\User\Desktop\data\info2.txt', "w" , encoding="utf8")  # write word to text file
    for i in range(x):
        for j in range(0, 3):
            w.write(doc[i][j]+";")
